...
public class ForLoop01 {
    ...
    public static void main(String[] args) {
        ...
        String pS = String.valueOf(p); // It converts the integer to String

        int sumOfUniqueDigit = 0;

        for (int i=0; i < pS.length(); i++) {

            char c = pS.charAt(i); // I am getting the character one by one

            if (pS.indexOf(c) == pS.lastIndexOf(c)) {// this code is for getting unique character   How to select the unique character. It is important !!!

                sumOfUniqueDigit = sumOfUniqueDigit + Integer.valueOf(c);
            }

        }

        System.out.println(sumOfUniqueDigit);
